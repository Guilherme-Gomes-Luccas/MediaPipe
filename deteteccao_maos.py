...
maos = mp_maos.Hands() #Modelos de deteccão de imagem

camera = cv2.VideoCapture(0)
resolucao_x, resolucao_y = 1280, 720

camera.set(cv2.CAP_PROP_FRAME_WIDTH, resolucao_x)
camera.set(cv2.CAP_PROP_FRAME_HEIGHT, resolucao_y)

bloco_notas, chrome, calc = False, False, False
...
while True:
    ...
    if len(todas_maos) == 1:
        ...
        if todas_maos[0]['lado'] == 'Right':
            ...
            if info_dedos_mao1 == [False, True, True, False, False] and calc == False:
                calc = True
                os.startfile(r'C:\Program Files\Google\Chrome\Application\chrome.exe')
            if info_dedos_mao1 == [False, True, True, True, False] and chrome == False:
                chrome = True
